#!/usr/bin/env bash
# src/modes/fix-zshrc.sh — fix zshrc mode (dedup, fix quotes, P10k compat)
# Sources: src/lib/helpers.sh must be sourced before
set -euo pipefail
...
CORE_PATH_LINE = 'export PATH="$HOME/.npm-global/bin:$HOME/.local/bin:$HOME/.n/bin:$HOME/.bun/bin:$HOME/.cargo/bin:$HOME/.jbang/bin:$HOME/.dotnet:$HOME/go/bin:/usr/local/go/bin:$PATH"'
