...
if grep -q "^ISOLATED_CIRCUIT=" "$CONFIG_FILE" 2>/dev/null; then
  _sed_i "s/^ISOLATED_CIRCUIT=.*/ISOLATED_CIRCUIT=$ISOLATED_CIRCUIT/" "$CONFIG_FILE"
else
...
if grep -q "^OPENCODE_LOCAL_ENDPOINT=" "$CONFIG_FILE" 2>/dev/null; then
  _sed_i "s|^OPENCODE_LOCAL_ENDPOINT=.*|OPENCODE_LOCAL_ENDPOINT=$OPENCODE_LOCAL_ENDPOINT|" "$CONFIG_FILE"
