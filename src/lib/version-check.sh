...
_check_versions() {
  ...
  if command -v node &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 --retry 3 --retry-delay 2 https://nodejs.org/dist/latest/SHASUMS256.txt 2>/dev/null | head -1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//' || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v python3 &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 --retry 3 --retry-delay 2 https://www.python.org/downloads/ 2>/dev/null | grep -oE 'Python 3\.[0-9]+\.[0-9]+' | sed 's/Python //' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v bun &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 --retry 3 --retry-delay 2 https://api.github.com/repos/oven-sh/bun/releases/latest 2>/dev/null | grep -oE '"tag_name": "bun-v[^"]+' | sed 's/"tag_name": "bun-v//' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v opencode &>/dev/null; then
    current=$(opencode --version 2>/dev/null | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    latest=$(curl -s --connect-timeout 5 --max-time 10 --retry 3 --retry-delay 2 https://api.github.com/repos/anomalyco/opencode/releases/latest 2>/dev/null | grep -oE '"tag_name": "v[^"]+' | sed 's/"tag_name": "v//' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ] && [ "$latest" != "$current" ]; then
  ...
  if command -v ollama &>/dev/null; then
    current=$(ollama --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown")
    latest=$(curl -s --connect-timeout 5 --max-time 10 --retry 3 --retry-delay 2 https://api.github.com/repos/ollama/ollama/releases/latest 2>/dev/null | grep -oE '"tag_name": "v[^"]+' | sed 's/"tag_name": "v//' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ] && [ "$latest" != "$current" ]; then
