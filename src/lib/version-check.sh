...
_check_versions() {
  ...
  if command -v rustc &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 'https://static.rust-lang.org/dist/channel-rust-stable.toml' 2>/dev/null | grep -A1 '\[pkg\.rustc\]' | grep version | awk -F'"' '{print $2}' | awk '{print $1}' || echo "$current")
  ...
  if command -v go &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 'https://go.dev/VERSION?m=text' 2>/dev/null | head -1 | tr -d 'go\n' || echo "$current")
  ...
  if command -v node &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://nodejs.org/dist/latest/SHASUMS256.txt 2>/dev/null | head -1 | grep -oP 'v\K[0-9]+\.[0-9]+\.[0-9]+' || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v python3 &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://www.python.org/downloads/ 2>/dev/null | grep -oP 'Python \K3\.[0-9]+\.[0-9]+' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v bun &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://api.github.com/repos/oven-sh/bun/releases/latest 2>/dev/null | grep -oP '"tag_name": "bun-v\K[^"]+' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
  ...
  if command -v opencode &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://api.github.com/repos/anomalyco/opencode/releases/latest 2>/dev/null | grep -oP '"tag_name": "v\K[^"]+' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ] && [ "$latest" != "$current" ]; then
  ...
  if command -v ollama &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://api.github.com/repos/ollama/ollama/releases/latest 2>/dev/null | grep -oP '"tag_name": "v\K[^"]+' | head -1 || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ] && [ "$latest" != "$current" ]; then
  ...
  if command -v zig &>/dev/null; then
    ...
    latest=$(curl -s --connect-timeout 5 --max-time 10 https://ziglang.org/download/index.json 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('master',{}).get('version',''))" 2>/dev/null || echo "$current")
    if [ "$current" != "$latest" ] && [ -n "$latest" ]; then
