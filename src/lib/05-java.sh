    ...
    if ! command -v zig &>/dev/null; then
      ...
      if [ ! -d "$ZIG_DIR" ]; then
        if _curl "$ZIG_URL" /tmp/zig.tar.xz 2>/dev/null; then
          ...
          log "Zig $ZIG_VER installed (direct)" || warn "Zig direct install failed"
        else
          warn "Zig $ZIG_VER not released yet, trying apt..."
          _sudo apt-get install -y -qq zig 2>/dev/null && log "Zig $(zig version 2>/dev/null) via apt" || warn "Zig unavailable"
        fi
        rm -f /tmp/zig.tar.xz
      else
