...
if pkg_installed("@modelcontextprotocol/server-github"):
    ...
    if gh_token:
        ...
        # Use env var reference instead of literal token (security fix)
        gh_entry["env"] = {"GITHUB_PERSONAL_ACCESS_TOKEN": "${GITHUB_PERSONAL_ACCESS_TOKEN}"}
    else:
...
def apply_mcp_profiles(config):
    ...
    for agent_name, agent_cfg in config.get("agent", {}).items():
        ...
        # Sort for deterministic output (sets are unordered; the diff-before-write
        # check requires byte-identical regeneration across runs).
        allowed = sorted(profile.get("mcp", []))
        perm = agent_cfg.setdefault("permission", {})
        for server in allowed:
            perm["mymcp_" + server] = "allow"
        if not full_mode:
            for hw in sorted(disabled):
...
# ── Diff-before-write (idempotency + dry-run) ───────────────────────────────
# 1) Serialize the proposed file to a canonical byte buffer.
# 2) SHA-256 the buffer; compare with the on-disk file (if any).
# 3) If equal → skip write, emit "UNCHANGED:<hash>", exit cleanly.
# 4) If different → emit a unified diff to stderr (masking secrets).
#    If DRY_RUN=1, do NOT write; otherwise write.
# Secrets (apiKey values starting with sk-/xai-/tp-/dtn_/ghp_/github_pat_) are
# never emitted in the diff — we substitute them with "<REDACTED:ENV>".
import hashlib, re, sys, difflib

header_lines = [
    "// ============================================================================",
    "// opencode.json — generated by opencode_initializer setup",
    "//",
    "// API key fields use ${ENV_VAR} substitution syntax.",
    "// Set the corresponding environment variable to activate a provider.",
    "// This file is safe to share — no secrets are stored in plain text.",
    "// ============================================================================",
]
proposed_json = json.dumps(config, indent=2)
proposed_bytes = ("\n".join(header_lines) + "\n" + proposed_json + "\n").encode("utf-8")
proposed_hash = hashlib.sha256(proposed_bytes).hexdigest()[:16]
...
if os.path.exists(config_path):
    ...
    existing_hash = hashlib.sha256(existing_bytes).hexdigest()[:16]

# Secret-masking regex: matches apiKey values like "sk-...abcd", "xai-...",
# "tp-...", "dtn_...", "ghp_...", "github_pat_..." — replace with <REDACTED:ENV>.
# Three patterns to cover:
#   JSON-style "key": "sk-..."  (key in quotes, value in quotes)
#   YAML-style key: "sk-..."   (key unquoted, value in quotes)
#   Env-var-value leak "KEY": "github_pat_..."  (env-var name in caps, leaked value)
_secret_re_json = re.compile(
    r'(\b(?:apiKey|password|token|secret)\b"\s*:\s*")(sk-|xai-|tp-|dtn_|ghp_|github_pat_)[A-Za-z0-9_-]+'
)
_secret_re_yaml = re.compile(
    r'(\b(?:apiKey|password|token|secret)\b\s*:\s*")(sk-|xai-|tp-|dtn_|ghp_|github_pat_)[A-Za-z0-9_-]+'
)
# Catch leaked values inside ANY quoted env-var field (e.g., GITHUB_PERSONAL_ACCESS_TOKEN).
_secret_re_env = re.compile(
    r'("[A-Z][A-Z0-9_]+"\s*:\s*")(sk-|xai-|tp-|dtn_|ghp_|github_pat_)[A-Za-z0-9_-]+'
)
def _mask(line: str) -> str:
    line = _secret_re_json.sub(r"\1\2<REDACTED:ENV>", line)
    line = _secret_re_yaml.sub(r"\1\2<REDACTED:ENV>", line)
    line = _secret_re_env.sub(r"\1\2<REDACTED:ENV>", line)
    return line

if existing_hash == proposed_hash:
    sys.stderr.write(f"opencode.json: UNCHANGED ({proposed_hash})\n")
    print(f"VALID:{len(mcps)}")
    sys.exit(0)

if existing_bytes:
    existing_lines = _mask(existing_bytes.decode("utf-8", "replace")).splitlines(keepends=True)
    proposed_lines = _mask(proposed_bytes.decode("utf-8", "replace")).splitlines(keepends=True)
    diff = difflib.unified_diff(
        existing_lines,
        proposed_lines,
        fromfile="opencode.json (current)",
        tofile="opencode.json (proposed)",
        n=3,
    )
    sys.stderr.write("".join(diff))

sys.stderr.write(f"opencode.json: DIFF detected ({existing_hash or '<none>'} -> {proposed_hash})\n")

dry_run = os.environ.get("DRY_RUN", "").lower() in ("true", "1", "yes")
if dry_run:
    sys.stderr.write("DRY-RUN: not writing opencode.json\n")
    sys.stdout.write(proposed_bytes.decode("utf-8"))
    sys.stdout.write("\n")
    print(f"DRY:{proposed_hash}")
    sys.exit(0)

with open(config_path, 'w') as f:
    f.write(proposed_bytes.decode("utf-8"))
...
PYGEN
  ...
  if echo "$RESULT" | grep -qE "^(VALID|DRY):"; then
    TAG=$(echo "$RESULT" | grep -oE "^(VALID|DRY):[0-9a-f]*" | head -1)
    if [ "${TAG#DRY:}" != "$TAG" ]; then
      # DRY-RUN path: print the proposed JSON (minus the trailing tag line)
      # to stdout so the caller can inspect or pipe it.
      log "opencode.json: DRY-RUN (no write) — ${TAG#DRY:}"
      # Find the line with "DRY:" and print everything before it as the proposed body.
      PROPOSED_BODY="$(printf '%s\n' "$RESULT" | awk '/^DRY:/{exit} {print}')"
      if [ -n "$PROPOSED_BODY" ]; then
        printf '%s\n' "$PROPOSED_BODY"
      fi
      # In dry-run, do NOT mark step done (we want a real run to write it).
    else
      MCP_COUNT=$(echo "$TAG" | cut -d: -f2)
