...
for f in "$PROJECT_DIR/lib/"*.sh "$PROJECT_DIR/modes/"*.sh; do
  fname=$(basename "$f")
  bash -n "$f" 2>/dev/null
...
HELPERS_LINE=$(grep -n 'lib/helpers.sh' "$PROJECT_DIR/setup.sh" | head -1 | cut -d: -f1)
CORE_LINE=$(grep -n 'lib/00-core.sh' "$PROJECT_DIR/setup.sh" | head -1 | cut -d: -f1)
...
LIB_COUNT=$(ls "$PROJECT_DIR/lib/"*.sh 2>/dev/null | wc -l)
assert "lib has modules" "[ '$LIB_COUNT' -ge 20 ]"

# ── Module naming convention: NN-name.sh ──────────────────────────────
for f in "$PROJECT_DIR/lib/"*.sh; do
  fname=$(basename "$f")
  if [ "$fname" = "helpers.sh" ]; then
    assert "$fname is helpers (no number)" "true"
  else
...
for mode in "$PROJECT_DIR/modes/"*.sh; do
  mname=$(basename "$mode")
...
assert "dev.sh sources helpers.sh" "grep -q 'lib/helpers.sh' '$PROJECT_DIR/dev.sh'"
assert "dev.sh has usage function" "grep -q 'usage()' '$PROJECT_DIR/dev.sh'"

# ── All files have set -euo pipefail ──────────────────────────────────
for f in "$PROJECT_DIR/setup.sh" "$PROJECT_DIR/dev.sh" "$PROJECT_DIR/lib/"*.sh "$PROJECT_DIR/modes/"*.sh; do
  fname=$(basename "$f")
...
assert "setup.sh is ~257 lines (+/- 10)" \
  "[ '$SETUP_LINES' -ge 247 ] && [ '$SETUP_LINES' -le 267 ]"

# ── Module count matches AGENTS.md description ────────────────────────
MODES_COUNT=$(ls "$PROJECT_DIR/modes/"*.sh 2>/dev/null | wc -l)
