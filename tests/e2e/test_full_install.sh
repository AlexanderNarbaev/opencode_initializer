...
# ── Test: Version string ──────────────────────────────────────────────
assert "version is v34.0" 'grep -q "v34.0" "'"$SETUP"'"'

# ── Test: set -euo pipefail in all files ──────────────────────────────
for f in "$PROJECT_DIR/setup.sh" "$PROJECT_DIR/lib/"*.sh "$PROJECT_DIR/modes/"*.sh; do
  [ -f "$f" ] || continue
  fname=$(basename "$f")
...
assert "trap cleanup exists" \
  'grep -q "trap cleanup" "'"$SETUP"'" || grep -q "trap cleanup" "'"$PROJECT_DIR/lib/helpers.sh"'"'

# ── Test: No hardcoded secrets ────────────────────────────────────────
SECRET_SCAN=$(grep -rni 'sk-[a-zA-Z0-9]\{20,\}' "$SETUP" "$PROJECT_DIR/lib/"*.sh 2>/dev/null || true)
...
HELPERS_LINE=$(grep -n 'lib/helpers.sh' "$SETUP" | head -1 | cut -d: -f1)
CORE_LINE=$(grep -n 'lib/00-core.sh' "$SETUP" | head -1 | cut -d: -f1)
assert "helpers sourced before core" "[ '$HELPERS_LINE' -lt '$CORE_LINE' ]"

# ── Test: setup.sh line count (~257) ─────────────────────────────────
SETUP_LINES=$(wc -l < "$SETUP")
assert "setup.sh is ~257 lines" "[ '$SETUP_LINES' -ge 247 ] && [ '$SETUP_LINES' -le 270 ]"
