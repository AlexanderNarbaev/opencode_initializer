...
# ── _curl caching behavior (skipped: requires network) ──────────────────────
assert "_curl caching skipped (no network needed)" "true"

# ── _npm_install exists ─────────────────────────────────────────────────
assert "_npm_install is defined" "type _npm_install &>/dev/null"

# ── Verify helpers.sh source structure ──────────────────────────────────
H="$PROJECT_DIR/src/lib/helpers.sh"
...
C="$PROJECT_DIR/src/lib/00-core.sh"
...
assert "00-core.sh has v2.0.0"         'grep -q "v2.0.0" "'"$C"'"'
