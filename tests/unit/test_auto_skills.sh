#!/usr/bin/env bash
# ============================================================================
# Unit Test for 53-auto-skills.sh — Auto-Triggering Skill System
# Target: src/lib/53-auto-skills.sh
# Session: ses_auto_skills_test
#
# Tests:
#   (a) Module file + syntax validity
#   (b) Core detection/suggestion functions present
#   (c) Structure & conventions (gates, opt-out, config.json write)
#   (d) macOS bash 3.2 compat (no associative arrays)
#   (e) Trigger keyword + file-pattern coverage
#   (f) Skill slugs referenced resolve on disk
#   (g) Functional detection (task type, file skills, dedupe, path resolution)
#   (h) config.json generation + validity
#
# Isolation: stubbed helpers (_step_skip/_step_done/section/log/info/warn);
# module sourced standalone with PROJECT_DIR set.
# ============================================================================
set -euo pipefail

PROJECT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
MODULE="$PROJECT_DIR/src/lib/53-auto-skills.sh"
...
assert "53-auto-skills.sh exists" "[ -f '$MODULE' ]"
assert "53-auto-skills.sh syntax clean" "bash -n '$MODULE'"

# ── 2. Core functions present ────────────────────────────────────────────
assert "has _detect_task_type()" "grep -q '_detect_task_type()' '$MODULE'"
assert "has _auto_load_skills()" "grep -q '_auto_load_skills()' '$MODULE'"
assert "has _skill_suggest()" "grep -q '_skill_suggest()' '$MODULE'"
assert "has _skills_for_type()" "grep -q '_skills_for_type()' '$MODULE'"
assert "has _detect_file_skills()" "grep -q '_detect_file_skills()' '$MODULE'"
assert "has _skill_path()" "grep -q '_skill_path()' '$MODULE'"
assert "has _dedupe()" "grep -q '_dedupe()' '$MODULE'"
assert "has _keyword_match()" "grep -q '_keyword_match()' '$MODULE'"
...
assert "has step_auto_skills skip check" "grep -q '_step_skip step_auto_skills' '$MODULE'"
assert "has _step_done step_auto_skills" "grep -q '_step_done step_auto_skills' '$MODULE'"
assert "has section header" "grep -q 'section.*Auto-Triggering Skills' '$MODULE'"
assert "has opt-out SKIP_AUTO_SKILLS" "grep -q 'SKIP_AUTO_SKILLS' '$MODULE'"
assert "config dir is ~/.config/opencode/auto-skills" "grep -q 'opencode/auto-skills' '$MODULE'"
assert "writes config.json" "grep -q 'config.json' '$MODULE'"
assert "no fabricated \$schema URL" "! grep -q 'opencode.ai/auto-skills.json' '$MODULE'"

# ── 4. macOS bash 3.2 compat — NO associative arrays ─────────────────────
assert "no declare -A (associative arrays)" "! grep -q 'declare -A' '$MODULE'"
...
for kw in specify define plan design test verify debug fix review refactor research; do
  assert "keyword '$kw' present in module" "grep -q '$kw' '$MODULE'"
done
for ext in '.ts' '.js' '.py' '.sh'; do
  assert "file pattern '$ext' present" "grep -qF '$ext' '$MODULE'"
done
...
for slug in specify plan brainstorm clarify running-tests writing-tests deep-research codebase-review-swarm; do
  assert "skill '$slug' exists" "[ -f '$PROJECT_DIR/.opencode/skills/$slug/SKILL.md' ]"
done
