...
cmd_docs() {
  ...
  emit "# Module Documentation (generated by \`dev docs\`)"
  emit "The RU column is maintained manually; EN is parsed from module headers."
  emit ""
  emit "| Module | Description (EN) | Описание (RU) |"
  emit "|---|---|---|"

  local f name desc
  for f in "$SCRIPTS_DIR/src/lib/"*.sh "$SCRIPTS_DIR/src/modes/"*.sh; do
    [ -f "$f" ] || continue
    name="$(basename "$f")"
    desc="$(grep -E '^# .*\.sh[[:space:]]+[—–-]' "$f" 2>/dev/null | head -n 1 | sed -E 's/^#[[:space:]]*//; s/^.*\.sh[[:space:]]*[—–-][[:space:]]*//')"
    emit "| \`$name\` | $desc |  |"
  done
}
