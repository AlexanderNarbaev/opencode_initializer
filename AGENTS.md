...
Модульная архитектура: 561-строчный оркестратор + 39 модулей + автообновление через systemd-таймер.
...
├── setup.sh          ← оркестратор (561 строк, source модули из src/lib/)
├── src/
...
```
opencode_initializer/
├── setup.sh              ← оркестратор (561 строк)
├── dev.sh                ← CLI
