...
### В: Модели Kimi/Moonshot таймаутят или возвращают пустые ответы
**О:** API Kimi имеет незадокументированный лимит ~20KB на тело запроса. Прокси `kimi-proxy` (v14.2+) автоматически сжимает большие payloads. Требования:

1. **VPN обязателен** — без VPN даже маленькие payloads могут таймаутиться при отправке
2. **Прокси должен работать** — `systemctl --user status kimi-proxy`
3. **Конфигурация** — прокси обрезает инструменты (макс. 10), сообщения (макс. 15) и описания (80 символов) для соблюдения лимита

Переменные окружения для тонкой настройки:
```bash
KIMI_PROXY_MAX_TOOLS=10        # Макс. инструментов на запрос (sticky идут первыми)
KIMI_PROXY_MAX_MSGS=15         # Последние N сообщений
KIMI_PROXY_MAX_MSG_CONTENT=400 # Макс. символов в контенте сообщения
KIMI_PROXY_MAX_TOOL_DESC=80    # Макс. символов в описании инструмента
KIMI_PROXY_MAX_BODY_KB=20      # Целевой размер payload
KIMI_PROXY_TIMEOUT=600         # Таймаут upstream в секундах
```

### В: Как использовать Kimi K3 в OpenCode?
**О:** Два варианта:

1. **Через прокси** (рекомендуется, автоматически обрабатывает лимит 20KB):
   - Модель: `moonshotai/kimi-k3`
   - Прокси на `http://127.0.0.1:9876/v1`

2. **Напрямую** (через `opencode auth login` → Moonshot AI):
   - Модель: `kimi-k3`
   - Прямое подключение к `https://api.moonshot.ai/v1`
   - Может столкнуться с лимитом 20KB при большом количестве инструментов
