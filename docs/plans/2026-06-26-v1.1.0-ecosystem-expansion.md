...
**Priority ranking:**
- **HIGH** (this session): OpenAI-compatible local API gateway, hardware auto-detection + optimal backend selection, lightweight CI/CD mode
...
## Task 1 [HIGH]: OpenAI-Compatible Local API Gateway (LiteLLM Proxy)
...
## Task 2 [HIGH]: Hardware Auto-Detection + Optimal Backend Selection
...
## Task 3 [HIGH]: Lightweight/Headless Mode for CI/CD and Scripts
...
After ALL tasks:
- [ ] `bash -n` passes on all `src/lib/*.sh src/modes/*.sh setup.sh dev.sh`
- [ ] `bash tests/run_tests.sh` passes all tests (updated assertions)
- [ ] ShellCheck passes on all .sh files
- [ ] `bash setup.sh --health` shows 65+ passed (up from 60+)
- [ ] `python3 -c "import json; json.load(open('~/.config/opencode/opencode.json'))"` succeeds
- [ ] `curl -s http://localhost:4000/health/liveliness` returns OK
- [ ] `opencode --version` works in CI mode
...
- [ ] All CLI flags documented in `setup.sh --help`
