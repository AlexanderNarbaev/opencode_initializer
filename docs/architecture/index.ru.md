...
OpenCode Initializer построен на модульной архитектуре: лёгкий **оркестратор** (`setup.sh`, 352 строки), который подключает 29 **модулей** и запускает 11 **режимов**.
...
```mermaid
flowchart TD
    A["setup.sh (352 строки)"] --> B["Определить SCRIPT_DIR"]
