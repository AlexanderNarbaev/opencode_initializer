...
OpenCode Initializer построен на модульной архитектуре: лёгкий **оркестратор** (`setup.sh`, 373 строки), который подключает 38 **модулей** и запускает 11 **режимов**.
...
C4Context
    ...
    System(oci, "OpenCode Initializer", "Настраивает полную dev-машину: 8 языков, 29 модулей, 21 MCP, 15 плагинов, инфраструктура")
...
C4Container
    ...
    Container_Boundary(oci, "OpenCode Initializer") {
        Container(setup, "setup.sh", "Bash", "Оркестратор — запускает 11 режимов, подключает 29 модулей, отслеживает прогресс")
        Container(dev_cli, "dev CLI", "Bash", "Управление после установки: install, remove, update, health, config")
        Container(lib, "src/lib/ (29 модулей)", "Bash", "Основные модули: система, языки, инструменты, MCP, LSP, LLM, провайдеры")
        Container(modes, "src/modes/ (5 скриптов)", "Bash", "Режимы: ci, health, fix-zshrc, upgrade, interactive")
        Container(tests, "tests/", "Bash + Bats", "Юнит, интеграционные, E2E тесты (193+ тестов)")
...
C4Container
    ...
    Container_Boundary(modules, "src/lib/") {
        ...
        Container(mise, "22-mise.sh", "Bash", "mise-en-place — менеджер версий инструментов")
        ...
        Container(providers, "26-providers.sh", "Bash", "Реестр 16 LLM-провайдеров")
...
```mermaid
flowchart TD
    A["setup.sh (373 строки)"] --> B["Определить SCRIPT_DIR"]
    B --> C["Подключить helpers.sh"]
    C --> D["Подключить 00-core.sh"]
    D --> E{"Разбор аргументов CLI"}
    E -->|"--health"| H["Режим диагностики (65+ проверок)"]
    ...
    M --> N["Последовательно: 01-system.sh … 28-devbox.sh"]
...
| **Мульти-провайдер** | 16 LLM-провайдеров с динамической регистрацией и переключением сессий |
| **LiteLLM шлюз** | OpenAI-совместимый эндпоинт для всех провайдеров — любые приложения используют ваши LLM |
