...
Файл `~/.cache/opencode-setup/progress` записывает выполненные шаги. Каждый модуль проверяет этот файл перед запуском:
...
Это обеспечивает идемпотентность — уже установленные компоненты пропускаются.
