...
## MCP-серверы (22 локальных + 2 удалённых)
...
| **notion** | `@notionhq/notion-mcp-server` | Платформа | Интеграция с Notion |
...
| **typescript** | TypeScript/JavaScript | Проверка типов, автодополнение, быстрые исправления |
| **pyright** | Python | Статическая проверка типов, автодополнение |
| **yaml** | YAML | Валидация схем, автодополнение, форматирование |
| **marksman** | Markdown | Проверка ссылок, генерация оглавления |
| **taplo** | TOML | Форматирование, валидация, автодополнение |
| **bash** | Bash/Shell | Анализ shell-скриптов, диагностика |
| **dockerfile** | Dockerfile | Подсветка синтаксиса, валидация |
| **css** | CSS | Валидация, автодополнение, форматирование |
| **html** | HTML | Валидация, автодополнение, форматирование |
| **json** | JSON | Валидация, автодополнение, форматирование |
...
| Плагин | Описание |
|--------|----------|
| **codegraph** | Анализ зависимостей и графа вызовов в редакторе |
| **dcp** | Damage Control Protocol — предотвращение опасных действий, сжатие контекста |
| **auto-fallback** | Автоматическое переключение провайдеров при ошибках |
| **goal-mode** | Структурированная разработка по целям с контрактами |
| **swarm** | Координация множественных агентов и ревью |
| **vibeguard** | Контроль качества перед завершением |
| **devcontainers** | Управление dev container воркспейсами |
| **worktree** | Изоляция веток через git worktree |
| **scheduler** | Фоновое планирование задач через systemd/launchd |
| **background-agents** | Асинхронное делегирование фоновым агентам |
| **goal-plugin** | Целе-ориентированное управление задачами |
| **conductor** | Оркестрация и планирование треков разработки |
| **zellij-namer** | Автоматическое именование сессий Zellij |
| **morph-plugin** | Контекстно-зависимая адаптация моделей |
| **supermemory** | Постоянная кросс-сессионная память для агентов |
| **websearch-cited** | Веб-поиск с инлайн-цитированием |
| **firecrawl** | Извлечение контента веб-страниц |
| **plugin-otel** | OpenTelemetry наблюдаемость и трейсинг |
| **token-tracker** | Учёт токенов и стоимости по сессиям |
| **orchestrator** | Оркестрация многошаговых задач |
| **daytona** | Интеграция Daytona для изолированных dev-окружений |
...
1. **MCP-сервер**: Добавить в `opencode.json` -> `mcp`
